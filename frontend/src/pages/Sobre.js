      ...
      <main style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div className="sobre-hero-inner" style={{ display: 'flex', alignItems: 'center', gap: '80px' }}>
            <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: '1.02rem', opacity: 0.5, maxWidth: '480px', lineHeight: '1.75', margin: 0 }}>
                Tem dúvidas sobre o ZapChat, quer uma demonstração ou precisa de um plano personalizado? Nossa equipe responde em até 4 horas.
              </p>
            </motion.div>

            {/* ── Logo SVG animado substituindo a imagem ── */}
        ...
        <section className="section-pad" style={{ padding: '0 10% 110px' }}>
          <div className="contact-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '70px', alignItems: 'start' }}>
            ...
            <motion.div initial={{ opacity: 0, x: -22 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: '800', marginBottom: '28px', letterSpacing: '-0.5px' }}>
                Canais de atendimento
              </h2>

              {[
                { icon: '💬', label: 'WhatsApp', value: '+55 (XX) XXXXX-XXXX', sub: 'Seg–Sex, 9h às 18h' },
                { icon: '📧', label: 'E-mail', value: 'contato@zapchat.com.br', sub: 'Resposta em até 4 horas' },
                { icon: '📍', label: 'Localização', value: 'São Paulo, SP — Brasil', sub: 'Atendimento 100% remoto' },
              ].map((item, i) => (
                <motion.div key={i}
                ...
                >
                  <div style={{ width: '42px', height: '42px', borderRadius: '12px', flexShrink: 0, background: 'rgba(37,211,102,0.08)', border: '1px solid rgba(37,211,102,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.05rem' }}>
                    {item.icon}
                  </div>
                  <div>
                    ...
                    <p style={{ fontWeight: '700', marginBottom: '2px', fontSize: '0.92rem' }}>{item.value}</p>
                    <p style={{ fontSize: '0.78rem', opacity: 0.32 }}>{item.sub}</p>
                  </div>
                </motion.div>
              ))}

              <div style={{ marginTop: '28px', padding: '22px', background: 'rgba(37,211,102,0.04)', borderRadius: '16px', border: '1px solid rgba(37,211,102,0.13)' }}>
                ...
                <p style={{ opacity: 0.45, fontSize: '0.83rem', lineHeight: '1.65', margin: 0 }}>
                  Somos uma equipe pequena e comprometida. Cada mensagem é lida e respondida pessoalmente.
                </p>
              </div>
            ...
            >
              {enviado ? (
                <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: 'center', padding: '40px 0' }}>
                  ...
                  <p style={{ opacity: 0.45, lineHeight: '1.65', marginBottom: '28px' }}>Recebemos seu contato e retornaremos em até 4 horas. Obrigado!</p>
                ...
                <>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: '800', marginBottom: '28px' }}>Envie uma mensagem</h3>
                  <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div>
                      ...
                      <textarea name="mensagem" value={form.mensagem} onChange={handleChange}
                        ...
                        rows={5} required style={{ ...inputStyle, resize: 'vertical', minHeight: '120px' }}
                      />
                    </div>
                    {erro && (
                    ...
                    <motion.button type="submit" disabled={enviando}
                      whileHover={{ scale: enviando ? 1 : 1.02, boxShadow: '0 0 28px rgba(37,211,102,0.2)' }} whileTap={{ scale: 0.98 }}
        ...
        <section className="section-pad" style={{ padding: '80px 10%', background: 'rgba(255,255,255,0.01)', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
          <div style={{ maxWidth: '680px', margin: '0 auto', textAlign: 'center' }}>
            <span style={{ color: '#25D366', fontWeight: '800', fontSize: '0.7rem', letterSpacing: '3px', textTransform: 'uppercase', display: 'block', marginBottom: '16px' }}>Sobre o ZapChat</span>
            <h2 style={{ fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', fontWeight: '900', letterSpacing: '-1px', marginBottom: '20px', lineHeight: '1.1' }}>
              Nascemos para resolver um problema real.
            </h2>
            <p style={{ opacity: 0.5, lineHeight: '1.8', fontSize: '0.95rem', marginBottom: '16px' }}>
              O ZapChat nasceu da frustração de ver pequenas e médias empresas perdendo clientes por não conseguir responder rápido o suficiente no WhatsApp. Criamos uma plataforma que qualquer pessoa consegue usar — sem código, sem técnico, sem complicação.
            </p>
            <p style={{ opacity: 0.5, lineHeight: '1.8', fontSize: '0.95rem', marginBottom: '36px' }}>
              Estamos no início da nossa jornada, construindo junto com nossos primeiros clientes. Cada feedback é levado a sério e cada melhoria vem de conversa real.
            </p>
