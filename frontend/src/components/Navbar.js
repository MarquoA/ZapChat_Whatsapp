    ...
    <>
      ...
      <nav style={{
        ...
        <Link to="/" style={{ textDecoration: 'none', color: 'inherit', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <h2 style={{ fontSize: '1.35rem', fontWeight: '900', letterSpacing: '-1px', margin: 0, lineHeight: 1 }}>
            ZAP<span style={{ color: '#25D366' }}>CHAT</span>
          </h2>
          <span style={{ background: 'rgba(37,211,102,0.12)', color: '#25D366', fontSize: '0.55rem', fontWeight: '800', padding: '2px 7px', borderRadius: '20px', border: '1px solid rgba(37,211,102,0.2)', textTransform: 'uppercase', letterSpacing: '1px' }}>BETA</span>
        </Link>
        ...
        <div className="nav-links-desktop" style={{ display: 'flex', gap: '36px', alignItems: 'center' }}>
          {links.map((link) => (
            <Link key={link.path} to={link.path} className="nav-link"
              style={{ textDecoration: 'none', color: isActive(link.path) ? '#25D366' : 'white', fontSize: '0.82rem', fontWeight: '600', letterSpacing: '0.3px', opacity: isActive(link.path) ? 1 : 0.5, position: 'relative' }}>
              {link.label}
              {isActive(link.path) && (
        ...
        {/* ── HAMBURGER MOBILE ── */}
        <button className="nav-hamburger" onClick={() => setMenuAberto(!menuAberto)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '5px', padding: '6px', alignItems: 'center', justifyContent: 'center' }}
          aria-label="Menu">
      ...
      <AnimatePresence>
        {menuAberto && (
          <motion.div
            ...
            style={{ position: 'fixed', top: '68px', left: 0, right: 0, zIndex: 999, background: 'rgba(6,10,6,0.98)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '24px 8%' }}>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '24px' }}>
              {links.map((link, i) => (
                <motion.div key={link.path} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                  <Link to={link.path}
                    style={{ display: 'block', textDecoration: 'none', color: isActive(link.path) ? '#25D366' : 'rgba(255,255,255,0.7)', fontSize: '1.05rem', fontWeight: '600', padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    {link.label}
